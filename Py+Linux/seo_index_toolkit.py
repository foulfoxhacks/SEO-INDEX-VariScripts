...
def calculate_score(engine: str, profile: dict[str, Any], factors: dict[str, FactorResult]) -> ScoreResult:
    ...
    for factor_name, weight_value in profile["weights"].items():
    ...
    normalized = round((earned / verified_max) * 100) if verified_max else 0
    capped = False
    if critical_failure and normalized > 49:
        normalized = 49
        capped = True
...
def print_scorecard(console: Console, score: ScoreResult) -> None:
    ...
    colored_score = console.paint(f"{score.normalized_score:3d}/100", Style.BOLD, score_color(score.normalized_score))
    print(f"  {colored_score}  {console.paint(progress_bar(score.normalized_score), score_color(score.normalized_score))}  {score.grade}")
    print(f"  Verified coverage: {score.coverage}% of the profile" + ("  • critical-failure cap applied" if score.capped else ""))
    print()
...
def write_markdown(path: str, snapshot: PageSnapshot, scores: list[ScoreResult]) -> None:
    ...
    for score in scores:
        lines.extend([
            f"## {score.label}: {score.normalized_score}/100 ({score.grade})",
            "",
            f"Verified profile coverage: {score.coverage}%.",
            "",
        ...
        try:
            ...
            if choice == "1":
                namespace = argparse.Namespace(
                    url=prompt("Page URL"),
                    engine=prompt("Engine: google, bing, generic, or all", "all").lower(),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    score.add_argument("--engine", choices=["google", "bing", "generic", "all"], default="all")
